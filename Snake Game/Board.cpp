...
void Board::Render(ID2D1HwndRenderTarget* _target)
{
	for (int i = 0; i < 12; i++)
		for (int j = 0; j < 8; j++)
			m_backSprite->Render(_target, D2D1::RectF((j * m_size.width / 2), i * m_size.height / 2, j * m_size.width / 2 + m_size.width / 2, i * m_size.height / 2 + m_size.height / 2));
			
	// 사과출력
	m_appleSprite->Render(_target, D2D1::RectF((m_appleXPos * m_size.width / 4), m_appleYPos * m_size.height / 2, m_appleXPos * m_size.width / 4 + m_size.width / 4, m_appleYPos * m_size.height / 2 + m_size.width / 4));
